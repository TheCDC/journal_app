...
class EntrySearchView(MethodView):
    def get_objects(self, **kwargs):
        ...
        found = list(
            flask_login.current_user.query_all_entries()
            .filter(models.JournalEntry.create_date >= start_date)
            .order_by(models.JournalEntry.create_date)
        )
        return found

    def args_to_date(self, year: int = None, month: int = None, day: int = None):
    ...
    def get(self, **kwargs):
        ...
        my_kwargs = {k: kwargs[k] for k in ["year", "month", "day"] if k in kwargs}
        try:
        ...
        e = (
            ...
            .filter(models.JournalEntry.id == e.id)
            .first()
        )
        ...
        return flask.render_template("entry.html", context=context)
...
class HomeView(MethodView, EnableLoggingMixin):
    ...
    def post(self, **kwargs):
        ...
        if upload_form.validate_on_submit():
            ...
            session = db.session()
            try:
                parsed = parsing.identify_entries(file.read().decode().split("\n"))
        ...
        if good_parse:
            ...
            return self.get(upload_form=upload_form, error="Invalid submission!")
        # return flask.redirect(flask.url_for('index'))
    ...
    def get(self, **kwargs):
        ...
        latest_entry = (
            models.JournalEntry.query.filter(
                models.JournalEntry.owner == flask_login.current_user
            )
            .order_by(models.JournalEntry.create_date.desc())
            .first()
        )
        ...
        context = dict(
            ...
            days_since_latest=(
                datetime.datetime.now().date() - latest_entry.create_date
            ).days
            if latest_entry
            else None,
            error=None,
    ...
    def get(self, **kwargs):
        ...
        if not form:
            form = forms.UserEditForm(
                **api.object_as_dict(flask_login.current_user),
                obj=flask_login.current_user,
            )
...
class EntryEditView(MethodView):
    ...
    def get(self, **kwargs):
        found = models.JournalEntry.query.filter_by(
            id=kwargs["id"], owner=flask_login.current_user
        ).first()
        if not found:
            flask.abort(404)
    ...
    def post(self, **kwargs):
        ...
        if form.validate_on_submit():
            if form.owner_id.data == cu.id:
                obj = models.JournalEntry.query.filter_by(
                    id=form.id.data, owner=cu
                ).first()
...
class EntryCreateView(MethodView):
    ...
    def get(self, **kwargs):
        context = dict(
            create=True,
            back=flask.url_for("home"),
            action=flask.url_for("create_entry"),
    ...
    def post(self, **kwargs):
        ...
        if form.validate_on_submit():
            ...
            return flask.redirect(
                api.link_for_entry(flask_login.current_user.get_latest_entry())
            )
...
class ExportJournalView(MethodView):
    ...
    def get(self, **kwargs):
        cu = flask_login.current_user
        entries = models.JournalEntry.query.filter_by(owner=cu).order_by(
...
def add_views(app):
    ...
    app.add_url_rule(
        "/delete/<int:id>", view_func=DeleteEntryView.as_view("delete_entry")
    )
