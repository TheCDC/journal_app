...
def count_occurrences(search):
    ...
    return all_entries.filter(
        webapp.models.JournalEntry.contents.contains(search)
    ).count()
...
class Plugin(classes.BasePlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.manager.blueprint.add_url_rule(
            self.endpoint,
            view_func=views.NameExtractorPluginView.as_view(
            ...
            try:
                if w[0].isalpha() and w[0] == w[0].upper():
                    if w not in seen:
                        ...
                        label = f"{w} ({c}),"
                        url = flask.url_for(
                            f"PluginManager.plugins-{self.safe_name}-index",
                            page=1,
                            search=w,
                        )
    ...
    def parse_entry(self, e: "webapp.models.JournalEntry") -> "iterable[str]":
        ...
        found = models.NameSearchCache.query.filter(
            models.NameSearchCache.parent == e
        ).first()
        if found:
