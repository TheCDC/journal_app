...
class User(db.Model, UserMixin):
    ...
    def get_latest_entry(
        ...
        # session.add(self)
        return (
            JournalEntry.query.filter(JournalEntry.owner_id == self.id)
            .order_by(JournalEntry.create_date.desc())
            .first()
        )
    ...
    def get_all_years(
        ...
        start_year = self.query_all_entries().order_by(JournalEntry.create_date).first()
        end_year = (
            self.query_all_entries().order_by(JournalEntry.create_date.desc()).first()
        )
        if start_year and end_year:
            for y in range(start_year.create_date.year, end_year.create_date.year + 1):
                ...
                found = (
                    ...
                    .filter(
                        JournalEntry.create_date >= datetime.datetime(y, 1, 1, 0, 0)
                    )
                    .filter(
                        JournalEntry.create_date < datetime.datetime(y + 1, 1, 1, 0, 0)
                    )
                    .first()
                )
    ...
    def next_entry(self, e: "JournalEntry") -> "JournalEntry":
        ...
        return (
            self.query_all_entries()
            .filter(JournalEntry.create_date > e.create_date)
            .first()
        )
...
class JournalEntry(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    create_date = db.Column(db.Date, default=datetime.datetime.utcnow, index=True)
    ...
    def next(self):
        found = (
            JournalEntry.query.filter(JournalEntry.owner_id == self.owner.id)
            .filter(JournalEntry.create_date > self.create_date)
            .first()
        )
        if found:
            if found.id != self.id:
...
if config.DEBUG_ENABLED:
    admin.add_view(
        JournalEntryView(JournalEntry, db.session, endpoint="model_view_journalentry")
    )
    admin.add_view(
...
class JournalEntrySchema(marshmallow.SQLAlchemyAutoSchema):
    class Meta:
        model = JournalEntry
        fields = (
            "id",
